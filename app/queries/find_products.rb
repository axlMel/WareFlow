class FindProducts
  ...
  def call(params = {})
    ...
    scoped = filter_by_min_stock(scoped, params[:min_stock])
    scoped = filter_by_max_stock(scoped, params[:max_stock])
  ...
  def filter_by_min_stock(scoped, min_stock)
    return scoped unless min_stock.present?

    scoped.where("stock >= ?", min_stock)
  end

  def filter_by_max_stock(scoped, max_stock)
    return scoped unless max_stock.present?

    scoped.where("stock <= ?", max_stock)
  end
